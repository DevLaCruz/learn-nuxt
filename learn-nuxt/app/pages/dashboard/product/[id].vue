...
import { longDateTimeFormat } from '~~/shared/utils/date-formats';
...
const tagsInput = ref(
  Array.isArray(product.value?.tags) ? product.value.tags.join(', ') : ''
);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const savedProduct = await createOrUpdate(newProduct.value, selectedFiles.value);

    if (isCreating.value) {
      router.replace(
        `/dashboard/product/${savedProduct.id}?message=Producto creado correctamente`
      );
      return;
    }
    ...
    toast.add({
      ...
      description: `El producto ${savedProduct.name} ha sido actualizado correctamente.`,
    });
  } catch (err: any) {
    toast.add({
      ...
      <form class="space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div class="space-y-3">
            ...
            <div v-if="newProduct.images && newProduct.images.length > 0" class="space-y-3">
              <div
            ...
            />
            <p class="text-sm text-gray-500 dark:text-gray-400">
              Selecciona imágenes para actualizar.
            </p>
        ...
        <dl class="mt-4 grid gap-4 sm:grid-cols-2">
          <div class="space-y-1">
            ...
            <dd class="text-gray-900 dark:text-gray-100">
              {{ longDateTimeFormat(product?.createdAt ? new Date(product.createdAt) : new Date()) }}
            </dd>
          </div>
          <div class="space-y-1">
            <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">
            ...
            <dd class="text-gray-900 dark:text-gray-100">
              {{ longDateTimeFormat(product?.updatedAt ? new Date(product.updatedAt) : new Date()) }}
            </dd>
          </div>
        </dl>
      </section>
